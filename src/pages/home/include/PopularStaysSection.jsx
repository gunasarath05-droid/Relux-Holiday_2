...
import { Star, MapPin, Zap, ArrowRight, Car } from 'lucide-react';
import { RESORTS_DATA } from '../../../data/resorts';
import BookingModal from '../../../components/common/BookingModal';

export default function PopularStaysSection() {
  const [selectedResort, setSelectedResort] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const featuredStays = RESORTS_DATA.filter((r) => r.featured);

  const handleOpenBooking = (resort) => {
          ...
          {featuredStays.map((resort) => (
            ...
            >
              ...
              <div className="relative aspect-[16/10] overflow-hidden bg-cream-200">
                ...
                {/* EV Charger Badge */}
                {resort.hasEvCharger && (
                  <div className="absolute bottom-4 left-4 px-2.5 py-1 rounded-full bg-brand-900/90 backdrop-blur-sm text-white text-[11px] font-semibold shadow-xs flex items-center gap-1">
                    <Zap className="w-3 h-3 text-brand-300 fill-brand-300" />
                    <span>Relux Fast Charger on-site</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 flex flex-col flex-grow justify-between space-y-4">
                ...
                <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs text-brand-800 font-semibold">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  ...
                  <button
                    onClick={() => handleOpenBooking(resort)}
